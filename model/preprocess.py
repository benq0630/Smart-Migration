...
def preprocess_data(df):
    ...
    if 'Google Rating' in df.columns:
        ...
        df['Google Rating'] = df['Google Rating'].clip(0, 5)  # 将评分限制在 0-5 之间
        df['Google Rating'] = df['Google Rating'].fillna(df['Google Rating'].median())  # 用中位数填充缺失值
    
    # 添加交互特征
    if 'Year of Experience' in transformed_df.columns and 'Google Rating' in df.columns:
        transformed_df['Experience_Rating_Interaction'] = transformed_df['Year of Experience'] * df['Google Rating']
    else:
